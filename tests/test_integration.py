...
class TestIntegration:
    ...
    def test_rule_processing_error_handling(self, temp_db):
        """Test error handling during rule processing"""
        # Create rules file with invalid rule
        invalid_rules = {
            "rules": [
                {
                    "name": "Invalid Rule",
        ...
        try:
            with patch('rule_engine.config.RULES_FILE', temp_file):
                ...
                temp_db.add(test_email)
                temp_db.commit()
                
                # Process rules - should handle errors gracefully
                engine = RuleEngine()
                engine.session = temp_db
                ...
                executions = temp_db.query(RuleExecution).filter_by(email_id='test_1').all()
                assert len(executions) >= 1
                assert executions[0].success is False  # Should be marked as failed
